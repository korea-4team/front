import { getReviewBoardRequest, patchReviewBoardRequest, uploadFileRequest } from 'apis';
import { REVIEW_BOARD_PATH } from 'constant';
import { PatchReviewBoardRequestDto } from 'interfaces/request/reviewBoard';
...
export default function ReviewBoardUpdate() {
  ...
  const [reviewBoardNumber, setReviewBoardNumber] = useState<string | null>('');
  ...
  const getReviewBoardResponseHandler = (responseBody: GetReviewBoardResponseDto | ResponseDto) => {
    ...
    const {businessType, location, title, contents, imageUrl} = responseBody as GetReviewBoardResponseDto;
    setReviewBoardTitle(title);
    setReviewBoardContents(contents);
    setReviewBoardImageUrl(imageUrl);
    setReviewBoardBusinessType(businessType);
    setReviewBoardLocation(location);
  ...
  // description : 게시물 수성 요청 함수 //
  const patchReviewBoardResponseHandler = (code: string) => {
  ...
  // description: 수정버튼 클릭 이벤트 //
  const onPatchButtonClickHandler = async () => {
    const token = cookies.accessToken;

    if (!boardNumber) return;

    const imageUrl = reviewBoardImage ? await fileUpload() : reviewBoardImage;
    ...
    patchReviewBoardRequest(boardNumber, data, token).then(patchReviewBoardResponseHandler);
  }
  ...
  useEffect(() => {
    ...
    setReviewBoardNumber(boardNumber);
  ...
  return (
    <div className="review-board-write-wrapper">
      ...
      <div className="review-board-write-button-box">
        ...
        <div className="black-button" onClick={onPatchButtonClickHandler}>수정</div>
      </div>
    </div>
  )
}
